...
gqpop <- read.csv(paste(path_data, 'Processed', 'gq_2000.csv', sep = delimiter_path)) %>%
		mutate(
...
stateferts <- read_csv(paste(path_data, 'Processed', 'state-level-fert-rates_20002020.csv', sep = delimiter_path))
	...
	tryCatch(
			{
				...
				predict = function(series, sex, x, DF) {
					#y <- as_data_frame(DF[[as.character(ccr)]][which(DF$COUNTYRACE == x & DF$SEX == sex)])
					y <- as_data_frame(DF[[as.character(series)]][which(DF$COUNTYRACE == x & DF$SEX == sex)])
					num <- seq(1, FORLEN, 5)
#					pred<- tryCatch(
...
foreach (i = 1:length(stateid), .combine = rbind, .errorhandling = "stop", .packages = pckgs) %dopar% {
	...
	write.csv(
			...
			paste(
					...
					'Projections',
					'Eval',
					paste(paste('COUNTY_20002020', stateid[i], sep = '_'), 'csv', sep = '.'),
					sep = delimiter_path
			),
			row.names = FALSE
	)
