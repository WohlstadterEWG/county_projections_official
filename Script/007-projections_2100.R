...
#
# Subject
#	Generate the Population Projections for the Period 2020 to 2100
#
# Purpose
...
gqpop <- read.csv(paste(path_data, 'Processed', 'gq_2010.csv', sep = delimiter_path)) %>%
		mutate(
...
stateferts <- read_csv(paste(path_data, 'Processed', 'state-level-fert-rates_20202100.csv', sep = delimiter_path))
	...
	tryCatch(
			{
				...
				predict = function(age, sex, x, series) {		# age is the age band
					y <- as_data_frame(series[[as.character(age)]][which(series$COUNTYRACE == x & series$SEX == sex)])
					num <- seq(1, FORLEN, 5)
#					pred<- tryCatch(
#							predict(ucm(value ~ 0, data = y, level = TRUE, slope = FALSE)$model, n.ahead = FORLEN)[c(num),],
#							error=function(e) array(0, c(STEPS))
#					)
					change <- tryCatch(
							forecast(arima(y$value, order = arima_order), h = FORLEN)$mean[c(num)],
							error = function(e) array(0, c(STEPS))
					)
				...
				CCDs <- K05 %>%
						ungroup() %>%
						mutate(
								AGE = paste0("X", str_pad(AGE, 2, pad = "0")),
								GEOID = paste0(STATE, COUNTY),
				...
				for (i in 1:(SIZE - 1)) {
					...
					data_tablef <- cbind(predict(paste0("ccr", i), "2", x, CCDs), 0, 0)
					#data_tablem <- cbind(predccr(paste0("ccr", i), "1", x, CCDs), 0, 0)
					data_tablem <- cbind(predict(paste0("ccr", i), "1", x, CCDs), 0, 0)
				...
				for (i in 1:SIZE) {
					popf[i] <- ifelse(
							length(K05$POPULATION[which(K05$SEX == "2" & K05$YEAR == launch_year & K05$AGE == i)]) == 0,
							0,
				...
				gqf <- ifelse(
						length(gqpop$group_quarters[which(gqpop$SEX == "2" & gqpop$COUNTYRACE == x)]) > 0,
						gqpop$group_quarters[which(gqpop$SEX == "2" & gqpop$COUNTYRACE == x)],
						0
				)
				...
				for(i in 1:SIZE) {
					popm[i] <- ifelse(
							length(K05$POPULATION[which(K05$SEX == "1" & K05$YEAR == launch_year & K05$AGE == i)]) == 0,
							0,
				...
				gqm <- ifelse(
						length(gqpop$group_quarters[which(gqpop$SEX == "1" & gqpop$COUNTYRACE == x)]) > 0,
						gqpop$group_quarters[which(gqpop$SEX == "1" & gqpop$COUNTYRACE == x)],
						0
				)
				...
				n01[, 1] <- n02$value[1:STEPS]				# n01 is three columns wide, but only first is populated.
				...
				for (i in 1:STEPS) {
					...
					projm <- projf <- array(0, c(SIZE, ncol(lx1f)), dimnames = list(paste0(rep('a', 18), 1:18)))
					...
					for (j in 1:ncol(lx1f)) {
						...
						# Derive newborn population from the state fertility rates * childbearing population * gender ratio
						projf[1, j] <- (n01[i, j] * sum(projf[4:10, j])) * 0.487
						projm[1, j] <- (n01[i, j] * sum(projf[4:10, j])) * 0.512
						# TODO Since only the first column of n01 is populated, projf/m[1, 2:3] is 0.  Is this correct?
...
#									'Projections',
#									'Proj',
#									paste(paste('proj', x, i, 'f', sep = '_'), 'csv', sep = '.'),
#									sep = delimiter_path
#							),
#							row.names = FALSE
#					)
...
#									'Projections',
#									'Proj',
#									paste(paste('proj', x, i, 'm', sep = '_'), 'csv', sep = '.'),
#									sep = delimiter_path
#							),
#							row.names = FALSE
#					)
...
#								'Projections',
#								'Proj',
#								paste(paste('projm', x, sep = '_'), 'csv', sep = '.'),
#								sep = delimiter_path
#						),
#						row.names = FALSE
#				)
...
#								'Projections',
#								'Proj',
#								paste(paste('projadd', x, sep = '_'), 'csv', sep = '.'),
#								sep = delimiter_path
#						),
#						row.names = FALSE
#				)
				...
				### Calculating the CCR UCMs for each individual age group (minus newborn)
				for (i in 1:(SIZE - 1)) {
					#data_tablef <- cbind(predccr(paste0("ccr", i), "2", x, CCRs), 0, 0)
					data_tablef <- cbind(predict(paste0("ccr", i), "2", x, CCRs), 0, 0)
					#data_tablem <- cbind(predccr(paste0("ccr", i), "1", x, CCRs), 0, 0)
					data_tablem <- cbind(predict(paste0("ccr", i), "1", x, CCRs), 0, 0)
				...
				for (i in 1:STEPS) {
					...
					projm <- projf <- array(0, c(SIZE, 1, ncol(lx1f)), dimnames = list(paste0(rep('a', 18), 1:18)))
...
#								'Projections',
#								'Proj',
#								paste(paste('proj', x, sep = '_'), 'csv', sep = '.'),
#								sep = delimiter_path
#						),
#						row.names = FALSE
#				)
...
test <- foreach(i = 1:length(stateid), .combine = rbind, .errorhandling = "stop", .packages = pckgs) %dopar% {
...
#					'Projections',
#					'Proj',
#					paste(paste('KT', stateid[i], sep = '_'), 'csv', sep = '.'),
#					sep = delimiter_path
#			),
#			row.names = FALSE
#	)
	...
	write.csv(
			...
			paste(
					...
					'Projections',
					'Proj',
					paste(paste('COUNTY_20202100', stateid[i], sep = '_'), 'csv', sep = '.'),
					sep = delimiter_path
			),
			row.names = FALSE
	)
