...
#						change<- tryCatch(
#								predict(
#										ucm(value ~ 0, data = data, level = TRUE, slope = FALSE)$model,
#										n.ahead = forecast_length
#								)[c(indices),],
#								error = function(e) array(0, cprojection_step_count)
#						)
						change <- tryCatch(
					...
					CCDs <- CCDs %>%
							...
							filter(
									as.integer(year) >= min(as.integer(year) + 5, na.rm = TRUE)
											& as.integer(year) <= as.integer(constants$analysis_year_start_projection)
							)
								...
								length(population_county$population[
												which(
														population_county$gender == "2"
																& population_county$year == constants$analysis_year_start_projection
					...
					for (i in 1:projection_step_count) {
						...
						#name_male <- get(paste("projection", i, "male", sep = '_'))
...
'
	...
	dbBind(select, list(year = constants$analysis_year_end_projection))
	...
	dbExecute(connection, 'DROP TABLE IF EXISTS "population__forecast__projection";')
...
'
	...
	foreach(i = 1:length(state_fips), .combine = rbind, .errorhandling = "stop", .packages = packages) %dopar% {
		...
		sql <- '
INSERT INTO "population__forecast__projection" (
